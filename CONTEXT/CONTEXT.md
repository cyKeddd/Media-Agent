...
**Shot kind**:
Each **Shot** is tagged `real_image` (a still — licensed or generated — animated image-to-video, ADR-0009) or `ai_video` (text-to-video transition/atmosphere, submitted straight to the configured video Provider with no still). A **Hybrid clip** mixes ~2 of each.
...
**Real-image shot**:
A **Shot** built from a **Licensed source** still of a product, logo, or object — never a living person — animated to video by the configured image-to-video Provider (ADR-0009; default `bytedance/seedance-2.0-fast`). Ken Burns motion is retained only as the fallback when image-to-video is unavailable, not the default.
_Avoid_: stock photo, slideshow frame.

**Licensed source**:
A rights-cleared origin for a **Real-image shot** still: brand logo APIs, Wikimedia, Openverse. Distinguished from open web-search results, which are not rights-cleared and are excluded from autonomous (auto-published) **Clips**. On a miss, ADR-0009's ladder falls to a **Generated still**; INV-7 forbids a **Generated still** from ever pre-empting a **Licensed source** that would have resolved.
_Avoid_: stock, free image, source.

**AI-video shot**:
A **Shot** submitted as text-to-video (no first-frame still) directly to the configured video Provider, for abstract/transitional b-roll between real-image shots. This is the ladder's last-resort rung (ADR-0009) — not the default path for a Shot naming a real entity.
_Avoid_: AI clip, generated segment, Kling shot (the provider is config-selected, not fixed to Kling).

**Generated still**:
An AI-generated still image produced by a **Still provider** and used as the first-frame conditioning input to an image-to-video call (ADR-0009). Produced when a **Real-image shot**'s **Licensed source** lookup misses, or when the **Shot** names no real entity. A **Generated still** renders a *plausible* frame, not *the* real thing, so it may never pre-empt a **Licensed source** hit (INV-7).
_Avoid_: AI image, generated photo, synthetic still.

**Still provider**:
The ABC (`src/image_gen/base.StillProvider`) that produces a **Generated still**. Production implementation: `NanoBananaProvider` (`google/gemini-3.1-flash-image`, ~$0.004/still). Swappable by config, mirroring the video **Provider** ABC.
_Avoid_: image generator, still model (use when naming the config key, not the concept).
...
per-**Shot** prompts sent to the configured video Provider). It is configured and run
