...
- [x] **Issue 59** (P1) — entry points `load_dotenv`; `bootstrap --check` validates key shape. `ba495a7`. Gate: 21 passed. `load_dotenv(ROOT/".env", override=False)` in `gen_run`/`daily_upload`/`bootstrap` before `load_config()`; key checked against `^sk-or-v1-[0-9a-f]{64}$`, failing (not warning) with distinct absent-vs-malformed messages and never echoing the value.
- [x] **Issue 60** (P1) — terminal run state; sweep runs abandoned > 90 min. `8fab1ee`. Gate: `test_run_finalizer.py` 15 passed. Sweep sits at `gen_run.py:656`, **inside** the lock taken at `:652`, so an in-flight run held by another process is never swept. Idempotent. The two real abandoned rows (2026-06-22, 2026-07-18) are deliberately left for the first live run rather than hand-mutated.
...
- [x] **Issue 62** (P1) — weekly 800¢ ceiling; per-clip cap reconciled to 150¢ (config drift 300 vs 250). `b4d327a`. Gate: 12 passed. **Root cause found first:** `generate_shots` recorded `quota_usage` only when `script_id` was truthy, so the ledger was *empty* and any cap built on it would read 0 forever and never fire — proven by `test_generate_shots_records_openrouter_quota` failing at pristine HEAD `7c0e246`. Fixed, then the rolling 7×24 h window (`recorded_at`, not the calendar `date` bucket) built on top. Boundaries asserted exactly: 799¢+1¢ allowed, 799¢+2¢ refused, zero provider calls on refusal.
- [x] **Issue 63** (P2) — `Provider.submit()` takes `first_frame_path`; `StillProvider` ABC. `f542092`. Gate: 57 passed (1 pre-existing failure, proven at pristine HEAD). A provider that cannot do i2v raises `UnsupportedFirstFrameError` rather than silently billing for an unconditioned render.
- [x] **Issue 64** (P2) — Nano Banana 2 still provider (≤5¢/still, ≤20¢/clip). Gate: 27 passed. Reuses `ai_gen.style_suffix` and `image_fetch.fetcher._reject_living_person` rather than duplicating either. Still spend is bucketed under `endpoint='openrouter_still'` so video spend cannot eat the 20¢ still allowance, while still summing into `quota_script_total` for INV-2.
- [x] **Issue 65** (P2) — Seedance 2.0 Fast image-to-video provider. `b79e372`. Gate: `test_seedance_provider.py` 28 passed. 4 s shot asserts to exactly 22¢.
- [x] **Issue 66** (P2) — image-first routing ladder; licensed source must not be pre-empted by a generated still. Gate: `test_shot_routing_ladder.py` 12 passed. INV-7 proven by call-**order** assertion. Also closed the INV-2 hole: `check_combined_clip_ceiling` covers video + still spend before every billable call.
- [x] **Issue 67** (P2) — flip runtime defaults + reconcile docs. Gate: `test_config_image_first.py` 17 passed. `build_video_provider` now called at `gen_run.py:445`, replacing the hardcoded Kling client (INV-10 real in the production path, not just in a test). Projection derives from the configured rate — the hardcoded 67¢ would have made the 800¢ ceiling behave like ~260¢.
- [x] **Issue 68** (P3) — subtitle/typography restyle. `aeeed6d`. Gate: 16 passed. Arial Black confirmed to resolve in libass (`fontselect: (Arial Black, 700, 0) -> Arial-Black`), evidence MP4 burned at 1080×1920.
- [x] **Issue 69** (P3) — freshness feeds (Techmeme, HN source role, Google News). `73dfc9b`. Gate: 15 passed. Four-source/four-URL/four-headline dedup collapses to one Topic.
- [ ] **Issue 70** (P1, HITL, **only issue with spend authority**, ≤150¢) — live resurrection run + spend reconciliation. **Still blocked** (see below).

### Known gaps carried forward (not silently closed)
- [ ] **`ai_video` shots do not traverse the ADR-0009 ladder.** `route_shot` handles that case and is tested in isolation, but `_generate_clip` still batches them straight to text-to-video; `route_shot` appears nowhere in `gen_run.py`. Real-image shots ARE image-first. Needs a follow-up ticket — rewiring it touches the concurrent-submission machinery in `ai_gen/runner.py` and the call-count assertions in `test_gen_run.py` / `test_hybrid_gen_run.py`.
- [ ] **OpenRouter wire schemas are documented assumptions, not verified.** The first-frame payload shape (Kling + Seedance) and the Nano Banana image request/response (`POST /api/v1/images`, `data[].b64_json`, `usage.cost`) are modelled on the closest documented precedent and locked by tests. They have never touched the live API. **Confirm before trusting Issue 70's first billed run.**
- [ ] **Blocked:** a real funded `OPENROUTER_API_KEY` must replace the placeholder in `.env` before Issue 70. Verified 2026-07-27: the value is **12 characters**, still the placeholder. `bootstrap --check` now fails loudly on this (Issue 59) instead of failing silently at billing time.
